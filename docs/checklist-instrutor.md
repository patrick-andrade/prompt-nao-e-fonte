...
- [ ] Se usar o zip opcional, conferir que exclui `aula/` e `instrutor/` (inclusive o `.qmd` e o HTML da autópsia). Para a Atividade 0, usar clone ou Download ZIP completo, pois o validador verifica essas pastas.
...
- [ ] Abrir `baixar_fm.R` antes de executar; mostrar entrada JSON, metadados e saída CSV. Rodar offline e validar com `uv run python scripts/validar_contrato.py` no ambiente preparado do professor.
- [ ] Retomar harness pela leitura de arquivos e execução de ferramentas. Mostrar onde estão registradas as escolhas e distinguir documentação, auditabilidade e reprodutibilidade.
...
- [ ] Atividade 3 (3 min): cada aluno escreve uma instrução com fonte/arquivo, resultado esperado e forma de conferência. Discutir por que conhecer R e Python ajuda a revisar código de IA.
