...
Nada disso exige programar hoje. Exige saber **o que é cada arquivo**, **quem lê o quê** — e a regra da aula: **ler antes de rodar**.

::: notes
Núcleo · bloco de 8 min começa aqui (1 min neste slide). Três verbos, sem enfeite. Voltar a eles no quiz do fecho.
:::
...
| git | clonar o repositório (sem git: **Download ZIP**) | quem tiver |
| Python (o do laboratório) | rodar o validador e `baixar_fm.py --offline`; só biblioteca padrão | todos <span class="degrau">degrau 3</span> |
| `uv` + `.venv` | isolar `pandas` / `matplotlib` para o `.qmd` e o lab de lacunas | para casa (`scripts/preparar_lab.py`) |
...
::: notes
Núcleo · 1 min. Ninguém cria conta em nada, ninguém instala biblioteca hoje. Aluno não precisa de Quarto nem de Netlify. RStudio existe no laboratório, mas o contrato mantém R só como menção. Dividir em voz alta: o laboratório fornece VS Code e Python; o repositório traz contrato, dado e scripts — próximo slide.
:::

## Regra da aula: ler antes de rodar

Nenhum comando "cola e roda". Antes de executar um arquivo, abrimos o arquivo, dizemos **o que** ele faz e **por que** vamos rodá-lo.

O primeiro arquivo que vamos rodar é o que **define o que é estar pronto**: o validador do contrato.

<p class="cue"><strong>Abrir agora:</strong> <code>scripts/validar_contrato.py</code> <span class="path-hint">(ler o cabeçalho, as constantes e os nomes das funções; ainda não rodar)</span></p>

::: {.tabela-compacta}
| Trecho do arquivo | O que faz, em uma frase | Por que importa |
| --- | --- | --- |
| Constantes do topo (`ISO3_CANONICO`, `INDICADORES`, `COLUNAS`, `VINTAGE_OK`) | Copiam para o código as cláusulas do `CONTRATO.md` | O contrato deixa de ser texto e vira teste |
| `validar_esqueleto()` | Confere que `CONTRATO.md` declara v1.2 e que as pastas do projeto existem | Sem o esqueleto, o resto não tem onde morar |
| `validar_csv()` | Abre o CSV com a biblioteca padrão (`csv`): colunas na ordem, só os 5 países, sem `CHN`, 2 indicadores, anos 2000–2029, vintage abril/2026, sem célula vazia nos anos-chave | É a autópsia do slop feita em código, linha por linha |
| `main()` | Imprime `STATUS: esqueleto OK` e `STATUS: CSV OK` — ou `FALHA:` com o motivo | Saída de exit 0 ou 1: um projeto real "passa" ou "não passa" |
:::

Só biblioteca padrão do Python. Nada para instalar.

::: notes
Núcleo · 3 min. Projetar o arquivo no VS Code: docstring do topo, depois as constantes (apontar `ISO3_CANONICO` e `ISO3_PROIBIDOS = {"CHN", "COL"}` — a China e a Colômbia proibidas em código), depois rolar pelos `def`. Não ler linha a linha; ler nomes e a lista de checagens. Frase-chave: "o mesmo contrato que lemos em Markdown está aqui como teste; quem quiser mudar país muda o contrato e o validador juntos". Ligar ao slide de extensões: `.md` é o contrato para pessoa e IA; `.py` é o contrato executável. O que foi pedido ao laboratório está em `docs/pedido-laboratorio.md`.
:::

## Atividade 0 · Onboarding *as is* {.slide-atividade}

Projeto real chega assim: um repositório e um validador. Agora que lemos o que ele checa, rodamos — e sabemos o que esperar.

::: {.atividade}
[3 min · individual · [degrau 2]{.degrau} → [degrau 3]{.degrau} · nada para instalar]{.atividade-meta}

1. Clonar `https://github.com/patrick-andrade/prompt-nao-e-fonte` (ou **Code → Download ZIP** no GitHub) e abrir a pasta no VS Code.
2. Terminal do VS Code, na raiz: `python scripts/validar_contrato.py` (ou **Terminal → Run Build Task**).
3. Esperado: `STATUS: esqueleto OK` e `STATUS: CSV OK`. Se aparecer `FALHA:`, a linha diz o que falta — leiam antes de chamar.
:::

O laboratório forneceu VS Code e Python. O repositório trouxe o resto: contrato, dado, scripts. Nenhuma biblioteca instalada.

::: notes
Núcleo · 3 min. Perguntar antes de rodar: "o que vai aparecer primeiro na tela?" (as duas primeiras linhas de `main()`: versão do contrato e a raiz). Quem está no degrau 1 acompanha pelo projetor. Nada de `uv` aqui: `pandas` e `matplotlib` só entram no `.qmd`, que é para casa (slide de gordura "Como um projeto puxa suas dependências").
:::
...
::: notes
Núcleo · bloco de 17 min começa aqui. Ler em voz alta o briefing (recorte + o que não fazer + prazo). Não ler o CONTRATO.md inteiro. México = LatAm já aparece no e-mail; China fora já aparece no e-mail.
:::
...
<p class="cue"><strong>Abrir agora:</strong> <code>02-dados-fiscal-monitor/scripts/baixar_fm.py</code> <span class="path-hint">(ler antes de rodar: onde está o `--offline`, o que ele lê em `data/raw/`, o que grava)</span></p>

::: notes
Núcleo · bloco de 13 min. Mesma regra: abrir o arquivo, mostrar a função que trata `--offline` e a que grava o CSV, e só então rodar no projetor. Dicionário: códigos, sem série. Se a rede do laboratório deixar, mencionar o 403 do DataMapper como motivo do cache.
:::
...
Degrau 3: rodem `python 02-dados-fiscal-monitor/scripts/baixar_fm.py --offline` e depois `python scripts/validar_contrato.py`. O CSV foi regravado a partir de `data/raw/` — a linha continua a mesma?
:::

::: notes
Núcleo · 5 min. É o "aha" da aula e tem de ser do aluno: Ctrl+F por `BRA,2025` ou `GGXONLB` funciona no VS Code e no GitHub. Contar as mãos; não ditar o valor nem copiá-lo para o deck. Quem está no degrau 1 usa o CSV na visualização do GitHub (o arquivo abre como tabela). O `--offline` já foi lido no slide anterior (regra: ler antes de rodar); só biblioteca padrão, nada a instalar.
:::
...
No clone vêm briefing, slop, CSV, `.qmd`, `docs/ferramentas.md` e este palco. **Não abrir** `instrutor/autopsia.md` antes do HTML das 17h42.

Produto no ar: **{{< meta params.url_netlify >}}** — demo do professor; aluno **não** cria conta.

Para casa: ler `scripts/preparar_lab.py`, rodá-lo (monta o `.venv` com `pandas` / `matplotlib`) e completar `03-relatorio-qmd/lab-lacunas.qmd`. Sem `uv`: as cinco perguntas de interpretação no fim do lab (degrau 2).
...
## Como um projeto puxa suas dependências [se sobrar tempo]{.selo-gordura}

O validador rodou com o Python do laboratório. O `.qmd` precisa de `pandas` e `matplotlib` — e um projeto real **não** pede para instalar à mão.

<p class="cue"><strong>Abrir agora:</strong> <code>scripts/preparar_lab.py</code> <span class="path-hint">(ler antes de rodar: cabeçalho e nomes das funções)</span></p>

::: {.tabela-compacta}
| Função | O que faz | Por que |
| --- | --- | --- |
| `degrau_2()` | Confere `CONTRATO.md`, `pyproject.toml`, `uv.lock` e o CSV | Sem contrato e dado, nada abaixo faz sentido |
| `localizar_uv()` / `instalar_uv()` | Procura o `uv`; se faltar, instala **só o executável** no perfil do usuário | Nada global; sem admin |
| `sincronizar()` | `uv sync --locked`: lê `pyproject.toml` + `uv.lock` e cria o `.venv` com as **mesmas versões** em toda máquina | O gráfico de quinta é o gráfico de hoje |
| `validar()` / `fechar()` | Roda o validador; imprime `DEGRAU: 2` ou `3` e a próxima ação | Falha vira instrução, não traceback |
:::

::: notes
Gordura (ou para casa). Ler pelos nomes; frase-chave: "`uv.lock` é a lista de versões congelada". Rodar só se a rede do laboratório deixar; senão, é o dever de casa para quem quiser completar o lab de lacunas.
:::
