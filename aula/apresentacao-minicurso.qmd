...
lang: pt-BR
params:
  url_netlify: "COLE-URL-DE-PRODUCAO"
...
Enquanto o parâmetro for `COLE-URL-DE-PRODUCAO`, o caminho local vale.
...
Se ainda for o placeholder: projetar o HTML local; depois do primeiro deploy verde, colar o `*.netlify.app` no YAML `url_netlify` e rerenderizar **este** deck.
...
2. Abrir o URL de produção do projeto `fiscal-monitor-2026` (git → Netlify).
3. Conferir no ar: 5 países, sem China, vintage no rodapé.
4. Se o YAML ainda tiver placeholder, colar o `*.netlify.app` em `url_netlify` e rerenderizar a aula.

Este deck **não** vai ao Netlify.
