...
Projeto real chega assim: um repositório, um README e um comando. Ninguém instala pacote à mão.
...
3. Deixar rodar. Esperado no fim: `STATUS: CSV OK` e `DEGRAU: 3`. Se parar em `DEGRAU: 2`, a aula segue igual no editor.
:::
...
::: notes
Núcleo · 2 min de fala; o `sync` roda em segundo plano enquanto lemos o briefing. É a primeira lição de projeto real: o repositório se instala sozinho (`pyproject.toml` + `uv.lock`); quem chega não "instala Python", roda o bootstrap. Sem admin, sem token. Quem estiver no degrau 1 acompanha pelo GitHub. O que foi pedido ao laboratório está em `docs/pedido-laboratorio.md`.
:::
