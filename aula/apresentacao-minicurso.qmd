...
date: "Semana da Economia · laboratório"
lang: pt-BR
params:
  url_netlify: "COLE-APOS-O-DROP"
format:
  revealjs:
...
::: notes
Núcleo · 5 min no bloco. Ainda não abrir o CSV nem o contrato inteiro. Deixar a cena (17h42 / 18h) no ar. Se alguém perguntar “é pra valer o tom da Diretoria?”: sim — no briefing e no produto; neste deck falamos com o estudante.
:::
...
<p class="cue"><strong>Abrir agora:</strong> <code>01-demanda-simulada/instrutor/autopsia.md</code> <span class="path-hint">(só o professor; fora do zip)</span></p>
...
O briefing interno fala *como* a Diretoria. Vocês deveriam ter entregue isto — não o dashboard das 17h42.

| Formato | Pasta | Destino |
| --- | --- | --- |
| Reveal.js | `outputs/revealjs-netlify/` | Único publish (Netlify) |
| PPTX | `outputs/pptx/` | Reunião interna; **não** se hospeda |
...
## No projetor: o briefing, não o slop

URL do produto (depois do drop): **{{< meta params.url_netlify >}}**

Enquanto o parâmetro for `COLE-APOS-O-DROP`, o caminho local vale.

<p class="cue"><strong>Abrir agora:</strong> <code>outputs/revealjs-netlify/</code></p>

Drop, quando for a hora: [Netlify Drop](https://app.netlify.com/drop) — conta do professor; aluno não cria conta.
...
::: notes
Núcleo. Fechar o bloco do produto. Gordura (lab, Drop ao vivo, r−g) fica para o final.
:::
...
## Zip do aluno, não o palco do professor

`aluno/minicurso-prompt-nao-e-fonte.zip`

**Não entra:** `aula/` (este deck), `instrutor/autopsia.md`, PPTX gerado, site Reveal.js.

<p class="cue"><strong>Abrir agora:</strong> <code>aluno/minicurso-prompt-nao-e-fonte.zip</code></p>

::: notes
Núcleo · 10 min finais. Conferir o zip na mão numa máquina se der (não precisa unzip no projetor).
:::

## Netlify é demo. Aluno não cria conta.

Publicado = conteúdo de `outputs/revealjs-netlify/`.

URL: **{{< meta params.url_netlify >}}**

Se ainda for o placeholder: render local + [Netlify Drop](https://app.netlify.com/drop), depois colar o `*.netlify.app` no YAML `url_netlify` e rerenderizar **este** deck.
...
## Drop Netlify ao vivo [se sobrar tempo]{.selo-gordura}

1. Render local do briefing (já deveria existir).
2. Abrir [Netlify Drop](https://app.netlify.com/drop) na **sua** conta.
3. Arrastar `outputs/revealjs-netlify/`.
4. Colar o `*.netlify.app` em `url_netlify` neste `.qmd` e rerenderizar a aula.

Este deck **não** vai para o Drop.

::: notes
Gordura. Se o drop já foi feito de manhã, só abrir o URL. Conferir no ar: 5 países, sem China, vintage no rodapé.
:::
