...
::: notes
Núcleo · bloco de 9 min começa aqui (1 min neste slide). Três verbos, sem enfeite. Voltar a eles no quiz do fecho.
:::
...
::: notes
Núcleo · 1 min. Ninguém cria conta em nada. Aluno não precisa de Quarto nem de Netlify. RStudio existe no laboratório, mas o contrato mantém R só como menção. Dividir em voz alta: o laboratório fornece VS Code, Python e rede; tudo o mais o projeto puxa — próximo slide.
:::
...
Projeto real chega assim: um repositório, um README, um bootstrap. Vamos ler o bootstrap.

<p class="cue"><strong>Abrir agora:</strong> <code>scripts/preparar_lab.py</code> <span class="path-hint">(ler o cabeçalho e os nomes das funções; ainda não rodar)</span></p>

::: {.tabela-compacta}
| Função no arquivo | O que faz, em uma frase | Por que importa |
| --- | --- | --- |
| `degrau_2()` | Confere se `CONTRATO.md`, `pyproject.toml`, `uv.lock` e o CSV estão no clone | Sem o contrato e o dado, nada abaixo faz sentido |
| `localizar_uv()` / `instalar_uv()` | Procura o `uv`; se faltar, instala **só o executável** no perfil do usuário | Nada global; ninguém precisa de admin |
| `sincronizar()` | `uv sync --locked`: lê `pyproject.toml` + `uv.lock` e cria o `.venv` com as **mesmas versões** em toda máquina | Reprodutibilidade: o gráfico de quinta é o gráfico de hoje |
| `validar()` | Roda `scripts/validar_contrato.py` | O projeto se declara pronto — ou diz o que falta |
| `fechar()` | Imprime `DEGRAU: 2` ou `3` e a próxima ação | Sem traceback; falha vira instrução |
:::

::: notes
Núcleo · 3 min. Projetar o arquivo no VS Code: docstring do topo (o que é, o que não é), depois rolar pelos `def` na ordem da tabela. Não ler código linha a linha — ler nomes. Frase-chave: "`uv.lock` é a lista de versões congelada; por isso o mesmo repositório dá o mesmo `.venv` em qualquer máquina". Ligar ao slide de extensões: `.py` executa, `.toml`/`.lock` configuram, `.md` é o contrato. O que foi pedido ao laboratório está em `docs/pedido-laboratorio.md`.
:::

## Atividade 0 · Onboarding *as is* {.slide-atividade}

Agora que lemos o que o script faz, rodamos — e sabemos o que esperar.

::: {.atividade}
[4 min · individual · dispara e segue · [degrau 2]{.degrau} → [degrau 3]{.degrau}]{.atividade-meta}

1. Clonar (ou baixar o zip do GitHub) e abrir a pasta no VS Code.
2. Terminal do VS Code, na raiz: `python scripts/preparar_lab.py` (ou **Terminal → Run Build Task**).
3. Acompanhar as mensagens na ordem das funções que acabamos de ler. Esperado no fim: `STATUS: CSV OK` e `DEGRAU: 3`. Se parar em `DEGRAU: 2`, a mensagem diz o que faltou e a aula segue no editor.
:::

O laboratório só forneceu VS Code, Python e rede. O resto o **projeto** puxa: `uv`, Python 3.13 do projeto, `pandas`, `matplotlib` — isolados na pasta do clone.

::: notes
Núcleo · 2 min de fala; o `sync` roda em segundo plano enquanto lemos o briefing. Perguntar antes de rodar: "o que vai aparecer primeiro na tela?" (a raiz do clone e o Python em uso — primeiras linhas de `main()`). É a primeira lição de projeto real: o repositório se instala sozinho; quem chega não "instala Python", roda o bootstrap que acabou de ler. Sem admin, sem token. Quem estiver no degrau 1 acompanha pelo GitHub.
:::
...
::: notes
Núcleo · bloco de 12 min. Não executar chunk neste deck. Mostrar o YAML (theme, reference-doc, footer com vintage) e um chunk que lê o CSV: o número do slide nasce ali.
:::
